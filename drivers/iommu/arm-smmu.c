...
static int arm_smmu_restore_sec_cfg(struct arm_smmu_device *smmu, u32 cb)
{
	...
	scm_ret = qcom_scm_restore_sec_cfg(smmu->sec_id, cb);
	if (scm_ret) {
				     ...
				     unsigned int nents, int prot)
{
	...
	ret = pgtbl_info->map_sg(ops, iova, sg, nents, prot, &size);
...
bool arm_smmu_skip_write(void __iomem *addr)
{
	...
	if (((unsigned long)addr & (smmu->numpage - 1)) < (smmu->numpage >> 1))
		return true;

	/* Finally skip writing to secure CB */
	cb = ((unsigned long)addr & ((smmu->numpage >> 1) - 1)) >> PAGE_SHIFT;
...
extern int qsmmuv2_halt(struct arm_smmu_device *smmu);
extern void qsmmuv2_resume(struct arm_smmu_device *smmu);
