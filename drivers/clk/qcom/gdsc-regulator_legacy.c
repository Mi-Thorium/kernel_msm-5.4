...
static int gdsc_enable(struct regulator_dev *rdev)
{
	...
	for (i = 0; i < sc->clock_count; i++) {
		if (unlikely(i == sc->root_clk_idx))
			continue;
		if (sc->toggle_mem)
			qcom_clk_set_flags(sc->clocks[i], CLKFLAG_RETAIN_MEM);
		if (sc->toggle_periph)
			qcom_clk_set_flags(sc->clocks[i], CLKFLAG_RETAIN_PERIPH);
	}
...
static int gdsc_disable(struct regulator_dev *rdev)
{
	...
	for (i = sc->clock_count - 1; i >= 0; i--) {
		...
		if (sc->toggle_mem && sc->allow_clear)
			qcom_clk_set_flags(sc->clocks[i], CLKFLAG_NORETAIN_MEM);
		if (sc->toggle_periph && sc->allow_clear)
			qcom_clk_set_flags(sc->clocks[i], CLKFLAG_NORETAIN_PERIPH);
	}
